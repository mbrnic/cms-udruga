...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$errorData = $Nav->checkForPostErrors($postData);
	
	if (!$errorData['errorFound']) $Nav->createNav($postData);
...
<main class="form-signin w-50 m-auto">
	<form method="POST" action="<?= ROOT_PATH . '/navigation/create' ?>">
		<br><br>

		<h1 class="h3 mb-3 fw-normal text-center">New navigation</h1>
...
<?php endif ?>
		...
		<div class="form-floating">
			<input type="text" class="form-control" id="text" name="name" placeholder="text" maxlength="50" required>
...
<?php
	$dbData = $Nav->whoCanBeParentNav();
	foreach ($dbData as $parent) {
...
/*
	$dbData = $Nav->getConnectionTypes();
	for ($i = 0; $i < count($dbData); $i++) {
			echo '<option value="' . $dbData[$i]['id'] . '"';
			if ($i == 0) echo ' selected';
			echo '>' . $dbData[$i]['type'] . '</option>';
	}
*/
?>
			</select>
		...
		<div class="form-floating">
			<input type="number" class="form-control" id="connection-id" name="connection-id" placeholder="text" min="1" value="1" required>
